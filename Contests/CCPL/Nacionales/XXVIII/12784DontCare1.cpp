...
bool converges(int n) {
    ...
    for(int i = 1; i < graph[n].size(); i++) {
        if(!conv[i]) if(!converges(graph[n][i])) return false;
        set<int> curr = dfsB(graph[n][i]);
        intersection = intersect(intersection, curr);
        if(intersection.size() == 0) return false;
    }
    conv[n] = true;
    for(int i = 0; i < graph[n].size(); i++) conv[graph[n][i]] = true;
    return true;
}
...
bool graphConverges(int n) {
    ...
    for(int i = 0; i < n; i++) {
        if(!conv[i] && !converges(i)) return false;
    }
    return true;
}
